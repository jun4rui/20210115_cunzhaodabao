    ...
    unreadMsgCount: function() {
      ...
      var _onMsg = this.conversations.map(function(item) {
        return {
          ...
          mtime: item.mtime,
          afkKey: item.username.toString() + '_' +
              item.unread_msg_count.toString(),//因为过来同一个消息发送人只有一条记录，这样离线自动回复会导致只自动回复一次。如果同一个人发送多次也仅仅只有一条记录，所以将发送人和未读消息数合在一起作为独特的KEY，用来保证同一个人每次发送都会被记录下来
      ...
      _onMsg.map(function(item) {
        ...
        if (mainModel.afkMode && item.mtime > mainModel.afkTime &&
  ...
  methods: {
    ...
    checkLoginStatus: function() {
      ...
      if (this.personUserInfo) {
        $.post('http://www.hnrcsc.com/web/seekjob/video!personLogin.action',
            function(response) {
      ...
      if (this.companyUserInfo) {
        $.post('http://www.hnrcsc.com/web/recruit/login!checkLogin.action',
            function(response) {
              if (response.map.status !== '00') {
                this.$notify.error({
                  title: '错误',
    ...
    autoResponse: function() {
      ...
      this.afkComeMessage.map(function(item, index) {
    ...
    checkDateTime: function() {
      ...
      if (!this.activityInfo) {
        ...
        if (_currentDateTime > _startDateTime && _currentDateTime <
            _endDateTime) {
          return true;
        } else {
          return false;
    ...
    getEndTime: function() {
      ...
      if (_holdingZone.indexOf('-') > -1 && _holdingZone.split('-').length ===
          2) {
        _startTime = _startTime.substr(0, 10) + ' ' +
            _holdingZone.split('-')[0] + ':00';
      }
    ...
    getActivityId: function() {
      ...
      var _tempActivityId = getParameterValue(window.location.href,
          'activityid')
    ...
    getActivityInfo: function() {
      //调用接口获取展会详情
      $.post(_SERVER + '/activity/getActivityById',
          {activityId: this.activityId}, function(response) {
            ...
            if (response.errCode === '00') {
              if (response.data === null) {
                ...
                //考虑到如果有holdingZone，则使用
                var _holdingZone = this.activityInfo.holdingZone;
                if (_holdingZone.indexOf('-') > -1 &&
                    _holdingZone.split('-').length === 2) {
                  this.activityTime = response.data.endDate.substr(0, 10) +
                      ' ' + _holdingZone.split('-')[1] + ':00';
                } else {
    ...
    getJobSeekerList: function() {
      $.post(_SERVER + '/activity/getOnsiteAndInvestList', {
        ...
        holdingTime: this.activityInfo ? this.activityInfo.holdingTime.substr(0,
            10) : '',//TODO 如果传companyid，就要传holdtime，这里应该要做一个判断
    ...
    addCounter: function() {
      ...
      var _parameter = {
        ...
        'type': '4-1-1',
        'url': 'http://www.hnrcsc.com/mianshibao/pageview/activityId/' +
            this.activityId,
        'ip': returnCitySN.cip,
    ...
    getCurrentCompanyInfo: function(inCompanyId) {
      ...
      $.post(_SERVER + '/wxCompany/companyMessageByWx', {
    ...
    getCurrentPositionInfo: function(inPositionId) {
      ...
      $.post(_SERVER + '/wxCompany/wxRecruitMessage', {recruitId: inPositionId},
          function(response) {
            if (response.errCode === '00') {
    ...
    getCurrentRecuitInfo: function(inRecruitId) {
      ...
      if (this.activityInfo.activityPattern ===
          1) _portUrl = '/wxCompany/wxRecruitMessage';
      if (this.activityInfo.activityPattern ===
          2) _portUrl = '/wxCompany/wxRecruitSiteMessage';

      $.post(_SERVER + _portUrl, {
        ...
        if (response.errCode === '00') {
          ...
          if (this.activityInfo.activityPattern ===
              1) this.currentRecruitInfo = response.data;
          if (this.activityInfo.activityPattern === 2) {
    ...
    viewResume: function(inCvId) {
      ...
      $.post('//www.hnrcsc.com/web' + '/recruit/view/cvJsonList.action?cvId=' +
          inCvId, function(response) {
    ...
    userLogin: function(inMode) {
      if (inMode === 'person') {
        window.localStorage.setItem('backurl',
            window.location.href.indexOf('?') > -1
                ...
                window.location.href.indexOf('?')) + '?mode=person'
      ...
      if (inMode === 'company') {
        window.localStorage.setItem('backurl',
            window.location.href.indexOf('?') > -1
                ? window.location.href.substr(0,
                window.location.href.indexOf('?')) + '?mode=company'
                : window.location.href +
                '?mode=company');
    ...
    viewMessage: function() {
      ...
      this.currentResumeInfo = null;
      if (this.personUserInfo) {
        this.chatToCompany(_tempTo.nickName,
            _tempTo.name.replace(/(^c|p)|(test$)/g, ''));
      }
      if (this.companyUserInfo) {
        this.chatToPerson(_tempTo.nickName,
            _tempTo.name.replace(/(^c|p)|(test$)/g, ''));
      }
    },
    ...
    chatViewPersonResume: function() {
      //获取用户简历列表
      $.post(_SERVER + '/personCenter/listPersonCv', {personId: this.targetId},
          function(response) {
            if (response.errCode === '00') {
              //获取用户默认简历ID
              var _tempCvId = 0;
    ...
    chatViewVideo: function() {
      ...
      if (this.personUserInfo !== null) {
        ...
        window.open('https://www.hnrcsc.com/videochat/client.html?personid=' +
            this.personUserInfo.personId + '&companyid=' + (this.targetId));
      }
      //企业方式
      if (this.companyUserInfo !== null) {
        ...
        window.open('https://www.hnrcsc.com/videochat/hr.html?companyid=' +
            this.companyUserInfo.companyId + '&orderid=' +
    ...
    postResume: function(inRecruitId) {
      ...
      if (this.activityInfo.activityPattern ===
          1) _interface = '/RecruitEdismax/recruitApply';
      if (this.activityInfo.activityPattern ===
          2) _interface = '/wxCompany/recruitApplySite';
    ...
    scroll2bottom: function() {
      ...
      _ele.scrollTo(
          {top: (_ele.scrollHeight - _ele.clientHeight), behavior: 'smooth'});
    },
  }, created: function() {
