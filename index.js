var mainModel = new Vue({
    ...
    computed: {
                ...
                if (response.errCode === '00') {
                    if (response.data === null) {
                        ...
                        if (_holdingZone.indexOf('-') > -1 && _holdingZone.split('-').length === 2) {
                            ...
                            this.activityTime = moment(response.data.endDate).set('hour', 23).set('minute', 59).set('second', 59).format('YYYY-MM-DD HH:mm:ss')
                        }
        ...
        userLogin: function (inMode) {
            ...
            if (inMode === 'company') {
                window.localStorage.setItem('backurl',
                    window.location.href.indexOf('?') > -1 ? window.location.href.substr(0, window.location.href.indexOf('?')) + '?mode=company' : window.location.href +
                        '?mode=company')
